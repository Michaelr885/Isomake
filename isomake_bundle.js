...
(function () {
    ...
    const EXPIRATION_DATE = new Date(atob('MjAyNi0wNS0yOVQxMDoxNTowMA==')).getTime();
...
let isCanvasMouseMoveScheduled = false;
let pendingCanvasMouseMoveEvent = null;

canvas.addEventListener('mousemove', (e) => {
    pendingCanvasMouseMoveEvent = e;
    if (!isCanvasMouseMoveScheduled) {
        isCanvasMouseMoveScheduled = true;
        requestAnimationFrame(() => {
            isCanvasMouseMoveScheduled = false;
            const e = pendingCanvasMouseMoveEvent;
            if (isPanning) return;
